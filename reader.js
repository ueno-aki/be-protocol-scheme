  ...
  const ctx = {
    ...
    Vec3fopts: (buffer, offset) => {
      ...
      let { value: z, size: zSize } = ((buffer, offset) => {
        ...
        return { value: undefined, size: 1}
      })(buffer, offset + xSize + ySize)
      return { value: { x, y, z }, size: xSize + ySize + zSize}
    },
    Vec2fopts: (buffer, offset) => {
      let { value: x, size: xSize } = ((buffer, offset) => {
        const {value} = ctx.bool(buffer, offset)
        if (value) {
          const { value, size } = (ctx.lf32)(buffer, offset + 1)
      ...
      return { value: { x, y }, size: xSize + ySize}
    },
    ...
    TransactionUseItem: (buffer, offset) => {
      ...
      let { value: face, size: faceSize } = (ctx.zigzag32)(buffer, offset + action_typeSize + block_positionSize)
      let { value: hotbar_slot, size: hotbar_slotSize } = (ctx.zigzag32)(buffer, offset + action_typeSize + block_positionSize + faceSize)
    ...
    ItemStackResponses: (buffer, offset) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        let { value: request_id1, size: request_id1Size } = (ctx.zigzag32)(buffer, offset + statusSize)
              ...
              for (let i = 0; i < count; i++) {
                ...
                let { value: slots, size: slotsSize } = ((buffer, offset) => {
                  ...
                  for (let i = 0; i < count; i++) {
                    ...
                    let { value: item_stack_id, size: item_stack_idSize } = (ctx.zigzag32)(buffer, offset + slot1Size + hotbar_slot1Size + count1Size)
    ...
    mcpe_packet: (buffer, offset) => {
      ...
      let { value: params, size: paramsSize } = ((buffer, offset) => {
        switch (name) {
          ...
          case "agent_animation": return (ctx.packet_agent_animation)(buffer, offset)
          case "refresh_entitlements": return (ctx.packet_refresh_entitlements)(buffer, offset)
          default: return (ctx.void)(buffer, offset)
        }
    ...
    packet_resource_packs_info: (buffer, offset) => {
      ...
      let { value: resource_pack_links, size: resource_pack_linksSize } = ((buffer, offset) => {
        const { value: count, size: countSize } = (ctx.varint)(buffer, offset)
        if (count > 0xffffff) throw new Error("array size is abnormally large, not reading: " + count)
        const data = []
        let size = countSize
        for (let i = 0; i < count; i++) {
          ...
          let { value: url1, size: url1Size } = (ctx.string)(buffer, offset + id1Size)
          return { value: { id: id1, url: url1 }, size: id1Size + url1Size}
        })(buffer, offset + size)
          data.push(elem.value)
          size += elem.size
        }
        return { value: data, size }
      })(buffer, offset + must_acceptSize + has_scriptsSize + force_server_packsSize + behaviour_packsSize + texture_packsSize)
      return { value: { must_accept, has_scripts, force_server_packs, behaviour_packs, texture_packs, resource_pack_links }, size: must_acceptSize + has_scriptsSize + force_server_packsSize + behaviour_packsSize + texture_packsSize + resource_pack_linksSize}
    },
    ...
    packet_start_game: (buffer, offset) => {
      ...
      let { value: editor_world_type, size: editor_world_typeSize } = ((buffer, offset) => {
        const { value, size } = (ctx.zigzag32)(buffer, offset)
        return { value: {"0":"not_editor","1":"project","2":"test_level"}[value] || value, size }
      })(buffer, offset + entity_idSize + runtime_entity_idSize + player_gamemodeSize + player_positionSize + rotationSize + seedSize + biome_typeSize + biome_nameSize + dimensionSize + generatorSize + world_gamemodeSize + difficultySize + spawn_positionSize + achievements_disabledSize)
      let { value: created_in_editor, size: created_in_editorSize } = (ctx.bool)(buffer, offset + entity_idSize + runtime_entity_idSize + player_gamemodeSize + player_positionSize + rotationSize + seedSize + biome_typeSize + biome_nameSize + dimensionSize + generatorSize + world_gamemodeSize + difficultySize + spawn_positionSize + achievements_disabledSize + editor_world_typeSize)
      let { value: exported_from_editor, size: exported_from_editorSize } = (ctx.bool)(buffer, offset + entity_idSize + runtime_entity_idSize + player_gamemodeSize + player_positionSize + rotationSize + seedSize + biome_typeSize + biome_nameSize + dimensionSize + generatorSize + world_gamemodeSize + difficultySize + spawn_positionSize + achievements_disabledSize + editor_world_typeSize + created_in_editorSize)
      let { value: day_cycle_stop_time, size: day_cycle_stop_timeSize } = (ctx.zigzag32)(buffer, offset + entity_idSize + runtime_entity_idSize + player_gamemodeSize + player_positionSize + rotationSize + seedSize + biome_typeSize + biome_nameSize + dimensionSize + generatorSize + world_gamemodeSize + difficultySize + spawn_positionSize + achievements_disabledSize + editor_world_typeSize + created_in_editorSize + exported_from_editorSize)
    ...
    packet_available_commands: (buffer, offset) => {
      ...
      let { value: chained_subcommand_values, size: chained_subcommand_valuesSize } = ((buffer, offset) => {
        const { value: count, size: countSize } = (ctx.varint)(buffer, offset)
        if (count > 0xffffff) throw new Error("array size is abnormally large, not reading: " + count)
        const data = []
        let size = countSize
      ...
      let { value: suffixes, size: suffixesSize } = ((buffer, offset) => {
        ...
        return { value: data, size }
      })(buffer, offset + values_lenSize + _enum_typeSize + enum_valuesSize + chained_subcommand_valuesSize)
      let { value: enums, size: enumsSize } = ((buffer, offset) => {
        ...
        return { value: data, size }
      })(buffer, offset + values_lenSize + _enum_typeSize + enum_valuesSize + chained_subcommand_valuesSize + suffixesSize)
      let { value: chained_subcommands, size: chained_subcommandsSize } = ((buffer, offset) => {
        const { value: count, size: countSize } = (ctx.varint)(buffer, offset)
        if (count > 0xffffff) throw new Error("array size is abnormally large, not reading: " + count)
        const data = []
        let size = countSize
        for (let i = 0; i < count; i++) {
          ...
          let { value: values, size: valuesSize } = ((buffer, offset) => {
            ...
            for (let i = 0; i < count; i++) {
              const elem = ((buffer, offset) => {
              let { value: index, size: indexSize } = (ctx.lu16)(buffer, offset)
              let { value: value2, size: value2Size } = (ctx.lu16)(buffer, offset + indexSize)
              return { value: { index, value: value2 }, size: indexSize + value2Size}
            })(buffer, offset + size)
              data.push(elem.value)
              size += elem.size
        ...
        return { value: data, size }
      })(buffer, offset + values_lenSize + _enum_typeSize + enum_valuesSize + chained_subcommand_valuesSize + suffixesSize + enumsSize)
      let { value: command_data, size: command_dataSize } = ((buffer, offset) => {
        ...
        for (let i = 0; i < count; i++) {
          ...
          let { value: chained_subcommand_offsets, size: chained_subcommand_offsetsSize } = ((buffer, offset) => {
            const { value: count, size: countSize } = (ctx.varint)(buffer, offset)
            if (count > 0xffffff) throw new Error("array size is abnormally large, not reading: " + count)
            const data = []
            let size = countSize
            for (let i = 0; i < count; i++) {
              const elem = (ctx.lu16)(buffer, offset + size)
              data.push(elem.value)
              size += elem.size
            }
            return { value: data, size }
          ...
          let { value: overloads, size: overloadsSize } = ((buffer, offset) => {
            ...
            for (let i = 0; i < count; i++) {
              const elem = ((buffer, offset) => {
              let { value: chaining, size: chainingSize } = (ctx.bool)(buffer, offset)
              let { value: parameters1, size: parameters1Size } = ((buffer, offset) => {
                ...
                for (let i = 0; i < count; i++) {
                  ...
                  let { value: enum_type, size: enum_typeSize } = ((buffer, offset) => {
                    const { value, size } = (ctx.lu16)(buffer, offset)
                    return { value: {"16":"valid","48":"enum","256":"suffixed","1040":"soft_enum"}[value] || value, size }
                ...
                return { value: data, size }
              })(buffer, offset + chainingSize)
              return { value: { chaining, parameters: parameters1 }, size: chainingSize + parameters1Size}
            })(buffer, offset + size)
              data.push(elem.value)
              size += elem.size
            }
            return { value: data, size }
          })(buffer, offset + name1Size + descriptionSize + flags1Size + permission_level1Size + aliasSize + chained_subcommand_offsetsSize)
          return { value: { name: name1, description, flags: flags1, permission_level: permission_level1, alias, chained_subcommand_offsets, overloads }, size: name1Size + descriptionSize + flags1Size + permission_level1Size + aliasSize + chained_subcommand_offsetsSize + overloadsSize}
        })(buffer, offset + size)
          data.push(elem.value)
          size += elem.size
        }
        return { value: data, size }
      })(buffer, offset + values_lenSize + _enum_typeSize + enum_valuesSize + chained_subcommand_valuesSize + suffixesSize + enumsSize + chained_subcommandsSize)
      let { value: dynamic_enums, size: dynamic_enumsSize } = ((buffer, offset) => {
        ...
        return { value: data, size }
      })(buffer, offset + values_lenSize + _enum_typeSize + enum_valuesSize + chained_subcommand_valuesSize + suffixesSize + enumsSize + chained_subcommandsSize + command_dataSize)
      let { value: enum_constraints, size: enum_constraintsSize } = ((buffer, offset) => {
        ...
        return { value: data, size }
      })(buffer, offset + values_lenSize + _enum_typeSize + enum_valuesSize + chained_subcommand_valuesSize + suffixesSize + enumsSize + chained_subcommandsSize + command_dataSize + dynamic_enumsSize)
      return { value: { values_len, _enum_type, enum_values, chained_subcommand_values, suffixes, enums, chained_subcommands, command_data, dynamic_enums, enum_constraints }, size: values_lenSize + _enum_typeSize + enum_valuesSize + chained_subcommand_valuesSize + suffixesSize + enumsSize + chained_subcommandsSize + command_dataSize + dynamic_enumsSize + enum_constraintsSize}
    },
    ...
    packet_update_soft_enum: (buffer, offset) => {
      let { value: enum_type, size: enum_typeSize } = (ctx.string)(buffer, offset)
      let { value: options, size: optionsSize } = ((buffer, offset) => {
        ...
        return { value: data, size }
      })(buffer, offset + enum_typeSize)
      let { value: action_type, size: action_typeSize } = ((buffer, offset) => {
        const { value, size } = (ctx.u8)(buffer, offset)
        return { value: {"0":"add","1":"remove","2":"update"}[value] || value, size }
      })(buffer, offset + enum_typeSize + optionsSize)
      return { value: { enum_type, options, action_type }, size: enum_typeSize + optionsSize + action_typeSize}
    },
    ...
    packet_education_settings: (buffer, offset) => {
      ...
      let { value: external_link_settings, size: external_link_settingsSize } = ((buffer, offset) => {
        switch (has_external_link_settings) {
          case true: return ((buffer, offset) => {
            let { value: has, size: hasSize } = (ctx.bool)(buffer, offset)
            let { value: url1, size: url1Size } = (ctx.string)(buffer, offset + hasSize)
            let { value: display_name1, size: display_name1Size } = (ctx.string)(buffer, offset + hasSize + url1Size)
            return { value: { has, url: url1, display_name: display_name1 }, size: hasSize + url1Size + display_name1Size}
          })(buffer, offset)
          default: return (ctx.void)(buffer, offset)
        }
    ...
    packet_completed_using_item: (buffer, offset) => {
      ...
      let { value: use_method, size: use_methodSize } = ((buffer, offset) => {
        const { value, size } = (ctx.li32)(buffer, offset)
        return { value: {"0":"equip_armor","1":"eat","2":"attack","3":"consume","4":"throw","5":"shoot","6":"place","7":"fill_bottle","8":"fill_bucket","9":"pour_bucket","10":"use_tool","11":"interact","12":"retrieved","13":"dyed","14":"traded","15":"brushing_completed"}[value] || value, size }
          ...
          case true: return ((buffer, offset) => {
            ...
            for (let i = 0; i < count; i++) {
              ...
              let { value: position1, size: position1Size } = ((buffer, offset) => {
                switch (action1) {
                  case "start_break": return (ctx.vec3i)(buffer, offset)
                  case "abort_break": return (ctx.vec3i)(buffer, offset)
                  case "crack_break": return (ctx.vec3i)(buffer, offset)
                  case "predict_break": return (ctx.vec3i)(buffer, offset)
                  case "continue_break": return (ctx.vec3i)(buffer, offset)
                  default: return (ctx.void)(buffer, offset)
                }
    ...
    packet_camera_presets: (buffer, offset) => {
      let { value: name, size: nameSize } = (ctx.string)(buffer, offset)
      let { value: parent, size: parentSize } = (ctx.string)(buffer, offset + nameSize)
      let { value: position, size: positionSize } = (ctx.Vec3fopts)(buffer, offset + nameSize + parentSize)
      let { value: rotation, size: rotationSize } = (ctx.Vec2fopts)(buffer, offset + nameSize + parentSize + positionSize)
      let { value: audio_listener, size: audio_listenerSize } = ((buffer, offset) => {
        const {value} = ctx.bool(buffer, offset)
        if (value) {
          const { value, size } = (ctx.u8)(buffer, offset + 1)
          return { value, size: size + 1 }
        }
        return { value: undefined, size: 1}
      })(buffer, offset + nameSize + parentSize + positionSize + rotationSize)
      let { value: player_effects, size: player_effectsSize } = ((buffer, offset) => {
        const {value} = ctx.bool(buffer, offset)
        if (value) {
          const { value, size } = (ctx.bool)(buffer, offset + 1)
          return { value, size: size + 1 }
        }
        return { value: undefined, size: 1}
      })(buffer, offset + nameSize + parentSize + positionSize + rotationSize + audio_listenerSize)
      return { value: { name, parent, position, rotation, audio_listener, player_effects }, size: nameSize + parentSize + positionSize + rotationSize + audio_listenerSize + player_effectsSize}
    },
    ...
    packet_camera_instruction: (buffer, offset) => {
      let { value: instruction_set, size: instruction_setSize } = ((buffer, offset) => {
        const {value} = ctx.bool(buffer, offset)
        if (value) {
          const { value, size } = ((buffer, offset) => {
          let { value: runtime_id1, size: runtime_id1Size } = (ctx.li32)(buffer, offset)
          let { value: ease_data, size: ease_dataSize } = ((buffer, offset) => {
            const {value} = ctx.bool(buffer, offset)
            if (value) {
              const { value, size } = ((buffer, offset) => {
              let { value: type1, size: type1Size } = ((buffer, offset) => {
                const { value, size } = (ctx.u8)(buffer, offset)
                return { value: {"0":"Linear","1":"Spring","2":"InQuad","3":"OutQuad","4":"InOutQuad","5":"InCubic","6":"OutCubic","7":"InOutCubic","8":"InQuart","9":"OutQuart","10":"InOutQuart","11":"InQuint","12":"OutQuint","13":"InOutQuint","14":"InSine","15":"OutSine","16":"InOutSine","17":"InExpo","18":"OutExpo","19":"InOutExpo","20":"InCirc","21":"OutCirc","22":"InOutCirc","23":"InBounce","24":"OutBounce","25":"InOutBounce","26":"InBack","27":"OutBack","28":"InOutBack","29":"InElastic","30":"OutElastic","31":"InOutElastic"}[value] || value, size }
              })(buffer, offset)
              let { value: duration1, size: duration1Size } = (ctx.lf32)(buffer, offset + type1Size)
              return { value: { type: type1, duration: duration1 }, size: type1Size + duration1Size}
            })(buffer, offset + 1)
              return { value, size: size + 1 }
            }
            return { value: undefined, size: 1}
          })(buffer, offset + runtime_id1Size)
          let { value: position1, size: position1Size } = ((buffer, offset) => {
            const {value} = ctx.bool(buffer, offset)
            if (value) {
              const { value, size } = (ctx.vec3f)(buffer, offset + 1)
              return { value, size: size + 1 }
            }
            return { value: undefined, size: 1}
          })(buffer, offset + runtime_id1Size + ease_dataSize)
          let { value: rotation1, size: rotation1Size } = ((buffer, offset) => {
            const {value} = ctx.bool(buffer, offset)
            if (value) {
              const { value, size } = (ctx.vec2f)(buffer, offset + 1)
              return { value, size: size + 1 }
            }
            return { value: undefined, size: 1}
          })(buffer, offset + runtime_id1Size + ease_dataSize + position1Size)
          let { value: facing, size: facingSize } = ((buffer, offset) => {
            const {value} = ctx.bool(buffer, offset)
            if (value) {
              const { value, size } = (ctx.vec3f)(buffer, offset + 1)
              return { value, size: size + 1 }
            ...
            return { value: undefined, size: 1}
          })(buffer, offset + runtime_id1Size + ease_dataSize + position1Size + rotation1Size)
          let { value: default1, size: default1Size } = ((buffer, offset) => {
            const {value} = ctx.bool(buffer, offset)
            if (value) {
              const { value, size } = (ctx.bool)(buffer, offset + 1)
              return { value, size: size + 1 }
            ...
            return { value: undefined, size: 1}
          })(buffer, offset + runtime_id1Size + ease_dataSize + position1Size + rotation1Size + facingSize)
          return { value: { runtime_id: runtime_id1, ease_data, position: position1, rotation: rotation1, facing, default: default1 }, size: runtime_id1Size + ease_dataSize + position1Size + rotation1Size + facingSize + default1Size}
        })(buffer, offset + 1)
          return { value, size: size + 1 }
        }
        return { value: undefined, size: 1}
      })(buffer, offset)
      let { value: clear, size: clearSize } = ((buffer, offset) => {
        const {value} = ctx.bool(buffer, offset)
        if (value) {
          const { value, size } = (ctx.bool)(buffer, offset + 1)
          return { value, size: size + 1 }
        ...
        return { value: undefined, size: 1}
      })(buffer, offset + instruction_setSize)
      let { value: fade, size: fadeSize } = ((buffer, offset) => {
        const {value} = ctx.bool(buffer, offset)
        if (value) {
          const { value, size } = ((buffer, offset) => {
          let { value: fade_in_duration, size: fade_in_durationSize } = (ctx.lf32)(buffer, offset)
          let { value: wait_duration, size: wait_durationSize } = (ctx.lf32)(buffer, offset + fade_in_durationSize)
          let { value: fade_out_duration, size: fade_out_durationSize } = (ctx.lf32)(buffer, offset + fade_in_durationSize + wait_durationSize)
          let { value: color_rgb, size: color_rgbSize } = (ctx.vec3f)(buffer, offset + fade_in_durationSize + wait_durationSize + fade_out_durationSize)
          return { value: { fade_in_duration, wait_duration, fade_out_duration, color_rgb }, size: fade_in_durationSize + wait_durationSize + fade_out_durationSize + color_rgbSize}
        })(buffer, offset + 1)
          return { value, size: size + 1 }
        }
        return { value: undefined, size: 1}
      })(buffer, offset + instruction_setSize + clearSize)
      return { value: { instruction_set, clear, fade }, size: instruction_setSize + clearSize + fadeSize}
    },
    ...
    packet_agent_animation: (buffer, offset) => {
      let { value: animation, size: animationSize } = ((buffer, offset) => {
        const { value, size } = (ctx.u8)(buffer, offset)
        return { value: {"0":"arm_swing","1":"shrug"}[value] || value, size }
      })(buffer, offset)
      let { value: entity_runtime_id, size: entity_runtime_idSize } = (ctx.varint64)(buffer, offset + animationSize)
      return { value: { animation, entity_runtime_id }, size: animationSize + entity_runtime_idSize}
    },
    packet_refresh_entitlements: (buffer, offset) => {
      return { value: {  }, size: 0}
    },
